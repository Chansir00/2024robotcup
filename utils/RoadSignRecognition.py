...
class RoadSignRecognition:
    def __init__(self, min_area = 50 * 50):
        ...
        self.results = []  # 存储近期的识别结果
        self.result_size = 10  # 只存储最近的5个结果
    ...
    def classify(self, image):
        ...
        if self.bbox is not None:
            ...
            if image[anchors[0][1], anchors[0][0]] >= 150 and image[anchors[0][3], anchors[0][2]] >= 150:
                self.road_sign = '1'
            elif image[anchors[1][1], anchors[1][0]] >= 150 and image[anchors[1][3], anchors[1][2]] >= 150:
                self.road_sign = '0'
        else:
            self.road_sign = None
    ...
    def update_results(self, result):
        ...
        if len(self.results) > self.result_size:
            self.results.pop(0)

    def most_common_sign(self):
        if self.results:
