...
class DangerSignRecognition:
    ...
    # x, y, x, y
    __class= np.array([
        [22,24,23,15,27,25,22,20,18,20] # fire 25, 38, 23, 37, 25, ,30
    ])
#22,24,23,15,27,25,15,42
    def classify(self, image):
        ...
        if self.bbox is not None:
            ...
            anchors = self.__class * np.array([scale, scale, scale,scale,scale]).reshape(-1)
            anchors = anchors.astype(np.int32)
            if image[anchors[0][1], anchors[0][0]] >= 80 and image[anchors[0][3], anchors[0][2]] >= 80 and image[anchors[0][5], anchors[0][4]] < 100 :
                if image[anchors[0][9],anchors[0][8]] < 100:
                    self.danger_sign="fall"
                else:
                    self.danger_sign = "explode" #白白黑白
            elif image[anchors[0][1], anchors[0][0]] < 100 and image[anchors[0][3], anchors[0][2]] >= 100 and image[anchors[0][5], anchors[0][4]] >= 100  :
                if image[anchors[0][7],image[0][6]] <100:
                    self.danger_sign = "collapse"
                else:
                    self.danger_sign = "water" #黑白白白
            elif image[anchors[0][1], anchors[0][0]] < 100 and image[anchors[0][3], anchors[0][2]] >= 100 and image[anchors[0][5], anchors[0][4]] < 100  :
                self.danger_sign = "fire" #黑白黑白
            # elif image[anchors[0][1], anchors[0][0]] < 100 and image[anchors[0][3], anchors[0][2]] < 100 and image[anchors[0][5], anchors[0][4]] < 100  :
            #     self.danger_sign = "fall" #黑白黑黑
            # elif image[anchors[0][1], anchors[0][0]] < 100 and image[anchors[0][3], anchors[0][2]] < 100 and image[anchors[0][5], anchors[0][4]] >= 100 :
            #         self.danger_sign = "collapse" #黑黑白黑
        else:
            self.danger_sign = None
